...
- Sprint 6 review: Fixed JSON output inconsistency — `grove list --json` used `"path"` for the worktree path field while `grove create --json` and `grove status --json` used `"worktree"`; renamed to `"worktree"` for consistency. Added roundtrip parse test to `grove init` — verifies the generated `.grove.yml` is parseable by `config.Parse`.
- Final review: Fixed path traversal vulnerability in `SanitizeBranchName` — branch names `..` and `.` were not sanitized, allowing `WorktreePath` to resolve outside the worktree directory (e.g., `..` resolved to `/`). Now maps `..` to `dotdot` and `.` to `dot`. Added test cases.

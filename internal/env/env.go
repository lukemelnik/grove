...
// IsManagedWorktreePath reports whether an untracked path has the exact shape
// of a Grove-owned env symlink or generated local file. It fails closed for
// invalid configuration paths, changed symlink targets, and unreadable files.
func IsManagedWorktreePath(cfg *config.Config, projectRoot, worktreePath, relativePath string) bool {
	if cfg == nil {
		return false
	}
	...
	rel := filepath.Clean(filepath.FromSlash(relativePath))
	dst, err := pathInsideRoot(worktree, rel)
	if err != nil {
		return false
	}

	for _, configured := range cfg.AllEnvFiles() {
		configured = filepath.Clean(configured)
		src, srcErr := pathInsideRoot(project, configured)
		if srcErr != nil {
			continue
		}
		...
		if rel == configured {
			...
			if filepath.Clean(targetAbs) == filepath.Clean(src) {
				return true
			}
			return sameSymlinkTarget(filepath.Dir(dst), target, src)
		}

		if rel == configured+".local" {
			info, err := os.Lstat(dst)
